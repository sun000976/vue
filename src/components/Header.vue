<template>
	<header class="header">
	  ...
	  <div class="navbar">
	    <div class="navbar-left-container" style="background: #fff;">
	    ...
	    <div class="navbar-right-container" style="display: flex;">
	      <div class="navbar-menu-container" style="position: absolute; right: 90px; top: 30px; font-size: 16px;">
	        ...
	        <a href="/" class="navbar-link" v-show="nickName" @click="logout">Logout</a>
	        <div class="navbar-cart-container" style="margin:5px 0 0 5px;width: 25px; height: 25px; display: inline-block;">
	  			...
	  			<div class="confirm-tips">
	  				...
	  				<ul>
	  					<li class="regi_form_input">
	  						...
	  						<input v-model="userName" type="text" tabindex="1" name="loginname" placeholder="User Name" class="regi_login_input regi_login_input_left">
	  					</li> 
	  					<li class="regi_form_input noMargin">
	  						<i class="icon IconPwd"></i> 
	  						<input v-model="userPwd" type="password" tabindex="2" name="password" placeholder="Password" @keyup.enter='login' class="regi_login_input regi_login_input_left login-input-no input_text">
	  					</li>
	  				</ul>
	  			</div> 
...
//			...mapState(['cartCount'])
		...
		methods:{
			checkLogin(){
				axios.get('/goods/checkLogin').then((response)=>{
			...
			logout(){
				axios.post('/goods/logout').then(response=>{
					let res = response.data;
					if (res.status=='0') {
						this.getCartCount();
						this.nickName = "";
			...
			getCartCount(){
				axios.get('/goods/getCartCount').then((response)=>{
